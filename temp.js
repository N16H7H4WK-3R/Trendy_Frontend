...
const Grid = (props) => {
    ...
    const [itemsToLoad, setItemsToLoad] = useState(18);
    const maxItems = 90;
    const uniqueIds = Array.from({ length: maxItems }, (_, idx) => idx + 1);
    ...
    const loadMoreItems = () => {
        ...
        const actualItemsToLoad = Math.min(newItemsToLoad, maxItems);
        ...
        <>
            ...
            >
                <Row xs={2} md={6} className="g-2 gy-4 my-2 fixRows" style={{ marginLeft: '20px', overflow: 'hidden' }}>
                    {uniqueIds.slice(0, itemsToLoad).map((id) => {
                        const product = props.productData[id - 1]; // Adjust index to match JSON data
                        return (
                            <Col key={id}>
